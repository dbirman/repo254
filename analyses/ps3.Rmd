...
```{r}
# This isn't really a histogram, but I don't know what the heck histogram you want. So I just did this because I understand the output.
d %>%
  group_by(age) %>%
...
```{r}
rs = lm(data=d,correct ~ age.group * condition)
```
...
```{r}
rs1 = lm(data=msi,mu~age.group * condition)
rs2 = lm(data=mss,mu~age.group * condition)
```
...
```{r}
rs = lmer(data=d,correct ~ age.group * trial * condition + (1|subid) + (1|trial))
```
...
```{r}
rs = lmer(data=d,correct ~ (1|subid))
```
