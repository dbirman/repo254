...
```{r data1}
dbad <- read.csv("C:\\Users\\Dan\\proj\\repo254\\data\\janiszewski_rep_exercise.csv")
```
...
```{r data2}
d <- read.csv("C:\\Users\\Dan\\proj\\repo254\\data\\janiszewski_rep_cleaned.csv")
```
...
```{r select}
d.tidy <- select(d,starts_with("Input."),starts_with("Answer."),starts_with("WorkerId"))
```
...
```{r rename}
d.tidy <- d.tidy %>% rename(.,cond=Input.condition,anchorDog=Input.price1,anchorPlasma=Input.price2,anchorSushi=Input.price3,estDog=Answer.dog_cost,estPlasma=Answer.plasma_cost,estSushi=Answer.sushi_cost,id=WorkerId)
d.tidy$id = as.numeric(d.tidy$id)
```
