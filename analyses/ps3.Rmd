...
rs = lm(data=d,correct ~ as.numeric(age.group) * condition)
summary(rs)
```
...
```{r}
rs1 = lm(data=msi,mu~as.numeric(age.group) * condition)
rs2 = lm(data=mss,mu~as.numeric(age.group) * condition)
```
...
```{r}
rs = lmer(data=d,correct ~ as.numeric(age.group) * condition + trial + (1+condition|subid) + (1|trial))
```

How do these coefficients compare with the independent coefficients linear model? What do you conclude?

Coeffs from lmer:
as.numeric(age.group)                    0.10071    0.03363   2.994
conditionNo Label                       -0.16729    0.09759  -1.714
trialfaces                              -0.08844    0.62197  -0.142
trialhouses                             -0.14966    0.62197  -0.241
trialpasta                              -0.01361    0.62197  -0.022
as.numeric(age.group):conditionNo Label -0.15800    0.04535  -3.484

From lm:
as.numeric(age.group)                    0.10042    0.03790   2.649  0.01539 *  
conditionNo Label                       -0.16732    0.11580  -1.445  0.16397    
as.numeric(age.group):conditionNo Label -0.15771    0.05360  -2.942  0.00806 ** 

For the carried over things, nothing much has changed, but now we additionally have information about the trials and an estimate of the random effects, suggesting that the between item variance is high:

 trial    (Intercept)       0.19222  0.4384        


Which random effects make the most difference? Find out using `ranef`. Plot the random effects for subject and item.

The items do, see above.
...
```{r}
rs3 = lmer(data=d,correct ~ (1|subid))
```
