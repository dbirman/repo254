...
```{r select}
d.tidy <- d.tidy %>% select(starts_with("Input."),starts_with("Answer."),starts_with("Worker"))
```
...
```{r gather}
d.tidy2 <- d.tidy1 %>% gather(est,estVal,5:7)
d.tidy3 <- d.tidy2 %>% gather(answer,ansVal,2:4)
d.long = d.tidy3
```
...
# For some reason this doesn't work...? It plots a plot that I wan't, but then it immediatley replcaes it with the scales='fixed' plot. If you know how to fix that it would be really helpful...
```
...
```{r}
d.long %>% summarise(mean(estVal,nan.rm=T))
```
