...
```{r prelims}
rm(list=ls())
source("C:\\Users\\Dan\\proj\\repo254\\helper\\useful.R")
```
...
```{r}
# Did outside cleanup of data by hand in a copied file
# - Fixing minor errors like typos
# - Searched for duplicate participants
# - Re-named several variables and got rid of unnecessary columns
```
...
```{r data2}
d.tidy <- read.csv("C:\\Users\\Dan\\proj\\repo254\\data\\janiszewski_rep_cleaned.csv")
```
...
```{r select}
d.tidy <- d.tidy %>% 
  select(starts_with("Input."),starts_with("Answer."),starts_with("Worker"))
```
...
```{r rename}
d.tidy1 <- d.tidy %>% rename(.,cond=Input.condition,anchorDog=Input.price1,anchorPlasma=Input.price2,anchorSushi=Input.price3,estDog=Answer.dog_cost,estPlasma=Answer.plasma_cost,estSushi=Answer.sushi_cost,id=WorkerId)
d.tidy1$id = as.numeric(d.tidy1$id)
```
...
```{r gather}
d.long <- d.tidy1 %>%
  gather(est,estVal,5:7) %>%
  gather(answer,ansVal,2:4) %>%
  mutate(group=answer) %>%
  filter(!is.nan(estVal)) # I thought this did what the next line does... for some reason it doesn' twork
d.long <- d.long[complete.cases(d.long),]
```

Bonus problem: *spread* these data back into a wide format data frame.

```{r spread}
d.wide <- d.tidy3 %>% spread(est,estVal) %>% spread(answer,ansVal)
```
...
```{r}
d.raw <- read.csv("C://Users//Dan//proj//repo254//data//janiszewski_rep_exercise.csv")
...
```{r}
d.long %>% ggplot +
  geom_histogram(aes(x=estVal)) +
  facet_grid(.~est,scale='free_x')

# The sushi esitmates and dog estimates look more or less 'normal', while the plasma estimates are clearly hitting a ceiling and skewed. If our analysis requires normality we might want to take this into account.
```
...
```{r}
d.long %>% group_by(est) %>% summarise(mean(estVal,nan.rm=T))
```
...
```{r}
d.long %>% group_by(cond,answer) %>% summarise(mean(estVal))
```
...
d.tidy <- d.unique.subs %>%
  ...
  spread(type, cost)
d.tidy$cost = as.numeric(gsub(",","",d.tidy$cost))
d.tidy$anchor = as.numeric(gsub(",","",d.tidy$anchor))
```
...
```{r}
pcts <- d.tidy %>%
  mutate(pct_change = (cost-anchor) / anchor ) %>%
  group_by(condition,item) %>%
  summarise(pct = mean(pct_change))
```
...
z.scores <- d.tidy %>%
  group_by(item) %>%
  mutate(z = (cost - anchor) / sd(anchor)) %>%
